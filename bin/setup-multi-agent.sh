#!/usr/bin/env bash
# agent-second-brain: multi-agent skill installer
...
#   - Codex CLI      : symlink to ~/.codex/skills/agent-second-brain
#   - OpenCode       : symlink to ~/.opencode/skills/agent-second-brain
#   - Gemini CLI     : symlink to ~/.gemini/skills/agent-second-brain
...
if [ ! -d "$SKILLS_DIR" ]; then
  echo "ERROR: $SKILLS_DIR does not exist. Are you running this from the agent-second-brain repo?"
  exit 1
fi
...
echo "agent-second-brain: multi-agent skill installer"
echo "Repo: $REPO_ROOT"
echo

# Codex CLI
link_if_missing "$SKILLS_DIR" "$HOME/.codex/skills/agent-second-brain" "Codex CLI"

# OpenCode
link_if_missing "$SKILLS_DIR" "$HOME/.opencode/skills/agent-second-brain" "OpenCode"

# Gemini CLI
link_if_missing "$SKILLS_DIR" "$HOME/.gemini/skills/agent-second-brain" "Gemini CLI"
...
echo "  - Codex CLI:   codex --list-skills | grep agent-second-brain"
